...
if __name__ == "__main__":
    ...
    if 'train' in args.mode:
        ...
        if args.world_size > 1:
            ...
            mp.spawn(train, args = (args,global_prefetch_step, end,True), nprocs=args.world_size, join=True)
        else:
            mgr = mp.Manager()
            end = mgr.Value('b', False)
            prefetch_step = mgr.list([0] * args.world_size)
            train(0,args,prefetch_step,end,load)

    if 'test' in args.mode:
        print('-------------test--------------')
        args.load_ckpt_name = os.path.join(args.model_dir, '{}-best.pt'.format(args.savename))
        if args.world_size > 1:
            mp.freeze_support()
            mgr = mp.Manager()
            end = mgr.Value('b', False)
