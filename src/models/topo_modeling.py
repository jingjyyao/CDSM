...
class TopoGramForNeighborPredict(GraphTuringNLRPreTrainedModel):
    ...
    def onestep_evaluate(self, input_ids_query, attention_masks_query, mask_query, input_ids_key, 
        ...
        query_ids = query_ids.expand(-1, neighbor_num+1, -1).reshape(batch_size*(neighbor_num+1), 1, -1) # B*(N+1) 1 L
        query_neighbor_ids = torch.cat([query_ids, input_ids_query.unsqueeze(1)], axis=1) # B*(N+1) 2 L
        query_neighbor_ids = query_neighbor_ids.reshape(batch_size*(neighbor_num+1)*2, -1) # B*(N+1)*2 L

        query_attention = attention_masks_query.view(batch_size, neighbor_num+1, -1)[:,:1,:] # B 1 L
        query_attention = query_attention.expand(-1, neighbor_num+1, -1).reshape(batch_size*(neighbor_num+1), 1, -1) # B*(N+1) 1 L
        query_neighbor_attention = torch.cat([query_attention, attention_masks_query.unsqueeze(1)], axis=1) # B*(N+1) 2 L
        query_neighbor_attention = query_neighbor_attention.reshape(batch_size*(neighbor_num+1)*2, -1) # B*(N+1)*2 L

        query_mask = torch.ones((batch_size*(neighbor_num+1), 1), dtype=mask_query.dtype, device=mask_query.device) # B*(N+1) 1
        neighbor_mask_query = torch.cat([query_mask, mask_query.unsqueeze(-1)], axis=1) # B*(N+1) 2


        key_ids = input_ids_key.view(batch_size, neighbor_num+1, -1)[:,:1,:] # B 1 L
        key_ids = key_ids.expand(-1, neighbor_num+1, -1).reshape(batch_size*(neighbor_num+1), 1, -1) # B*(N+1) 1 L
        key_neighbor_ids = torch.cat([key_ids, input_ids_key.unsqueeze(1)], axis=1) # B*(N+1) 2 L
        key_neighbor_ids = key_neighbor_ids.reshape(batch_size*(neighbor_num+1)*2, -1) # B*(N+1)*2 L

        key_attention = attention_masks_key.view(batch_size, neighbor_num+1, -1)[:,:1,:] # B 1 L
        key_attention = key_attention.expand(-1, neighbor_num+1, -1).reshape(batch_size*(neighbor_num+1), 1, -1) # B*(N+1) 1 L
        key_neighbor_attention = torch.cat([key_attention, attention_masks_key.unsqueeze(1)], axis=1) # B*(N+1) 2 L
        key_neighbor_attention = key_neighbor_attention.reshape(batch_size*(neighbor_num+1)*2, -1) # B*(N+1)*2 L

        key_mask = torch.ones((batch_size*(neighbor_num+1), 1), dtype=mask_key.dtype, device=mask_key.device) # B*(N+1) 1
        neighbor_mask_key = torch.cat([key_mask, mask_key.unsqueeze(-1)], axis=1) # B*(N+1) 2   

        
        hidden_states_query = self.bert(query_neighbor_ids, query_neighbor_attention,
                                             neighbor_mask=neighbor_mask_query,
                                             mask_self_in_graph=False,
                                             return_last_station_emb=False
                                             )
        hidden_states_key = self.bert(key_neighbor_ids, key_neighbor_attention,
                                           neighbor_mask=neighbor_mask_key,
                                           mask_self_in_graph=False,
                                           return_last_station_emb=False
                                       )
        last_hidden_states_query = hidden_states_query[0] # B*(N+1)*2 L+1 D
        last_hidden_states_key = hidden_states_key[0] # B*(N+1)*2 L+1 D

        #delete the station_placeholder hidden_state:(N,1+L,D)->(N,L,D)
        last_hidden_states_query = last_hidden_states_query[:,1:]  # B*(N+1)*2 L D
        last_hidden_states_key = last_hidden_states_key[:, 1:] # B*(N+1)*2 L D

        #hidden_state:(B*(N+1),L,D)->(B,N,D)
        new_query_embed = last_hidden_states_query[::2][:,0].view(batch_size, 1+neighbor_num, -1) # B 1+N D
        new_key_embed = last_hidden_states_key[::2][:,0].view(batch_size, 1+neighbor_num, -1) # B 1+N D

        neighbor_mask_query = mask_query.view(batch_size, neighbor_num+1) # B 1+N
        neighbor_mask_key = mask_key.view(batch_size, neighbor_num+1) # B 1+N
    ...
    def inference(self, input_ids_query, attention_masks_query, mask_query, input_ids_key, attention_masks_key, mask_key, 
        neighbor_num, aggregation='mean', mask_self_in_graph=False, return_last_station_emb=False):
        '''
        Args:
            input_ids: Tensor(batch_size*(neighbor_num+1),seq_length)
        '''
        ...
        #hidden_state:(N,L,D)->(B,L,D)
        query = last_hidden_states_query[::(neighbor_num+1)][:,0] # B D
        key = last_hidden_states_key[::(neighbor_num+1)][:,0] # B D
