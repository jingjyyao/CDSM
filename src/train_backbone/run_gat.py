...
import utils
from models.GAT_modeling import GATForNeighborPredict
...
def load_bert(args):
    ...
    model = GATForNeighborPredict.from_pretrained(args.model_name_or_path,
...
def test_single_process(model, args, mode):
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(dataloader):
            ...
            query = model.gat(node_embed_query, neighbor_mask_query)
            node_embed_key = last_hidden_states_key[:, 0].view(batch_size, 1 + args.neighbor_num, -1)
            key = model.gat(node_embed_key, neighbor_mask_key)
...
def test(local_rank, args, global_prefetch_step, end):
    ...
    for step, batch in enumerate(dataloader):
        ...
        node_embed_query = last_hidden_states_query[:, 0].view(batch_size, 1 + args.neighbor_num, -1)
        query = ddp_model.gat(node_embed_query, neighbor_mask_query)
        node_embed_key = last_hidden_states_key[:, 0].view(batch_size, 1 + args.neighbor_num, -1)
        key = ddp_model.gat(node_embed_key, neighbor_mask_key)
