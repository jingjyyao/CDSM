...
def load_bert(args):
    ...
    model = GraphSageForNeighborPredict.from_pretrained(args.model_name_or_path,
...
def test_single_process(model, args, mode):
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(dataloader):
            ...
            node_embed_query = last_hidden_states_query[:, 0].view(batch_size, 1 + args.neighbor_num, -1)
            query = model.graphsage(node_embed_query, neighbor_mask_query, args.aggregation)
            node_embed_key = last_hidden_states_key[:, 0].view(batch_size, 1 + args.neighbor_num, -1)
            key = model.graphsage(node_embed_key, neighbor_mask_key, args.aggregation)
        ...
        if args.mlm_loss:
            logging.info('Final-- mlm_acc:{}, qk_acc:{}'.format(mlm_acc[0] / mlm_acc[1],
                                                                retrive_acc[0] / retrive_acc[1]))
        else:
...
def test(local_rank, args, global_prefetch_step, end):
    ...
    for step, batch in enumerate(dataloader):
        ...
        node_embed_query = last_hidden_states_query[:, 0].view(batch_size, 1 + args.neighbor_num, -1)
        query = ddp_model.graphsage(node_embed_query, neighbor_mask_query, args.aggregation)
        node_embed_key = last_hidden_states_key[:, 0].view(batch_size, 1 + args.neighbor_num, -1)
        key = ddp_model.graphsage(node_embed_key, neighbor_mask_key, args.aggregation)
